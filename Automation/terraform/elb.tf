...
"Statement": [
    ...
    "Action": "s3:PutObject",
    "Effect": "Allow",
    "Resource": "arn:aws:s3:::${var.prefix}-tg-tf-log/*",
    "Principal": {
        "AWS": ["797873946194"]
    }
    }
...
resource "aws_elb" "tungsten_elb" {
  ...
  instances                   = ["${aws_instance.tungsten.*.id}"]
  #instances                   = [ "${element(aws_instance.tungsten.*.id, count.index)}"]
  #instances                   = "${lookup(aws_instance.tungsten,count.index)}"
